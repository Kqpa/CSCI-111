...
public class LetterSquare {
    ...
    {        
        ...
        {
            return !onSameSide(letter, lastLetter(this.words[wordNum]))
    ...
    {
        if (allLettersUsed() && dictionary.hasFullWord(this.words[wordNum]) && wordNum + 1 == maxWords)
        {
            printSolution(words.length - 1);
            return true;
        }
        ...
        for (int i = 0; i < letters.length; i++)
        {
            if (isValid(letters[i], wordNum, charNum))
            {
                ...
                else if (words[wordNum].length() >= 3
                        && dictionary.hasFullWord(words[wordNum])
